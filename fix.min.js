...
b.document.title="Google Drive Picker"})})}function f(g){return Promise.resolve().then(function(){return fetch(g,{method:"GET",headers:new Headers({"X-Requested-With":"XMLHttpRequest"})})}).then(function(b){return b.text()}).then(function(b){if(b=b.split("\n").find(function(l){return-1!==l.indexOf("var oauthToken =")}))return b.split(" = ")[1].trim().slice(1,-2)})}c.addEventListener("message",function(g){if(g.data.drivePickerData&&(g=g.data.drivePickerData,g.action==google.picker.Action.PICKED)){Element.show("tb_google_drive_stub");
var b=jQuery("#google_drive_files_list").data("google-drive-document-data"),l=[];void 0===b&&(b=[]);g.docs.each(function(a){document_ids=b.map(function(d){return d.id});-1==jQuery.inArray(a.id,document_ids)&&(l.push(a),b.push(a))});present_selected_files(l);jQuery("#google_drive_files_list").data("google-drive-document-data",b)}});c.openPickerWindow=function(){e().catch(function(g){MD_alert("<h1>Error</h1><p>"+g)})}})(window);
